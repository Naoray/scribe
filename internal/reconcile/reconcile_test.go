...
func TestReconcileProjectRootRemovesOrphanedGlobalClaudeProjection(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	...
	globalPath := filepath.Join(home, ".claude", "skills", "recap")
	if err := os.MkdirAll(filepath.Dir(globalPath), 0o755); err != nil {
		t.Fatalf("MkdirAll global: %v", err)
	}
	if err := os.Symlink(canonical, globalPath); err != nil {
	...
	projectRoot := t.TempDir()
	projectPath := filepath.Join(projectRoot, ".claude", "skills", "recap")
	if err := os.MkdirAll(filepath.Dir(projectPath), 0o755); err != nil {
		t.Fatalf("MkdirAll project: %v", err)
	}
	if err := os.Symlink(canonical, projectPath); err != nil {
	...
	st := &state.State{SchemaVersion: 4, Installed: map[string]state.InstalledSkill{
		"recap": {
			...
			Tools:        []string{"claude"},
			ToolsMode:    state.ToolsModePinned,
			ManagedPaths: []string{globalPath, projectPath},
			Projections:  []state.ProjectionEntry{{Project: projectRoot, Tools: []string{"claude"}}},
		},
	}}
	...
	if _, err := os.Lstat(projectPath); err != nil {
		t.Fatalf("project projection missing: %v", err)
	}
	for _, path := range st.Installed["recap"].ManagedPaths {
		if path == globalPath {
			t.Fatalf("ManagedPaths retained removed global path: %v", st.Installed["recap"].ManagedPaths)
...
func TestReconcileProjectRootPreservesTrackedGlobalClaudeProjection(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	...
	st := &state.State{SchemaVersion: 4, Installed: map[string]state.InstalledSkill{
		"scribe": {
			...
			ToolsMode:    state.ToolsModePinned,
			ManagedPaths: []string{globalPath},
			Projections:  []state.ProjectionEntry{{Project: "", Tools: []string{"claude"}}},
		},
	}}
